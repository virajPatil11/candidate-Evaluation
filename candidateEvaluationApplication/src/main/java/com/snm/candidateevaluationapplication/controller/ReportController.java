...
//		excelReportGenerator.generateReportByLevel(evaliations, level);
	 ...
	 */	
	@RequestMapping("/generateReportByEvaluator/{empName}")
    public ResponseEntity<ByteArrayResource> generateReportByEvaluator(@PathVariable String empName) throws IOException {
		List<Feedback> feedbacks  = reportService.findByEvaluator(empName);
		
		// Generate the Excel report
        byte[] excelFile = excelReportGenerator.generateReportByEvaluator(feedbacks, empName);
        
        // Set the response headers to indicate that it's a file download
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        headers.setContentDispositionFormData("attachment", "candidate_evaluation_report_by_feedback_status_" + empName + ".xlsx");
